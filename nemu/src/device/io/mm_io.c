...
static int nr_map = 0;         //Number of the io map
...
void* add_mmio_map(uint32_t addr, size_t len, mmio_callback_t callback) {
	...
	maps[nr_map].low = addr;                 //start addr of the io
	maps[nr_map].high = addr + len - 1;		 //end addr of the io
	maps[nr_map].mmio_space = space_base;    //io space
...
void clear_mmio_map() {   //set all of the io clear
	nr_map = 0;
