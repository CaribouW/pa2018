...
inline uint32_t internal_normalize(uint32_t sign, int32_t exp, uint64_t sig_grs)
{
	...
	uint32_t sticky = 0;
	if ((sig_grs >> (23 + 3)) > 1 || exp < 0) // 1xxxx.xxx or exp<0 , we have to move rightWard.
	{
		...
		if (exp == 0) //Too small and is the denormal one.
		{
		...
		if (exp < 0) //overflow,occurs in the div operation.
		{
	...
	else if (((sig_grs >> (23 + 3)) == 0) && exp > 0) //The bit is less than 26.  0.00xxxx.....
	{
	...
	else if (exp == 0 && sig_grs >> (23 + 3) == 1) //The 1 means the carry to the front of the dot   1.xxxxx
	{
	...
	if (!overflow)
	{
		...
		uint32_t grs = (sig_grs & 0x7); // 0111
		if (grs > 0x4)
		{
			sig_grs += 0x8;
		}
		else if (grs == 0x4) //0100
		{
			if ((sig_grs >> 3) & 0x1) //not even
			{
				sig_grs += 0x8;
			}
		}
...
uint32_t internal_float_add(uint32_t b, uint32_t a)
{
	...
	//let the exp of b bigger than a.
	...
	uint32_t shift = (fb.exponent == 0 ? fb.exponent + 1 : fb.exponent) -
					 (fa.exponent == 0 ? fa.exponent + 1 : fa.exponent);
	assert(shift >= 0);

	sig_a = (sig_a << 3); // guard, round, sticky
	sig_b = (sig_b << 3); //get three bit more.
	...
	// fraction add , before the adding we have to put the sig into positive
	if (fa.sign)
	{
		sig_a *= -1;//get complement
	}
	if (fb.sign)
	{
		sig_b *= -1;//get complement
	}
	//Translate the code into complement code.
