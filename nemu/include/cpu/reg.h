#ifndef __REG_H__
#define __REG_H__

#include "nemu.h"
typedef struct
{
	uint32_t limit : 16;
	uint32_t base : 32; //The base addr of the GDT
						//base + index<<3 is the addr of the des
} GDTR;

typedef struct
{
	uint32_t limit : 16;
	uint32_t base : 32;
} IDTR;
/**
 * control register
*/
typedef union {
	struct
	{
		uint32_t pe : 1;
		uint32_t mp : 1;
		uint32_t em : 1;
		uint32_t ts : 1;
		uint32_t et : 1;
		uint32_t reserve : 26;
		uint32_t pg : 1;
	};
	uint32_t val;
} CR0;

typedef union {
	struct
	{
		uint32_t reserved : 12;
		uint32_t PDBR : 20;
	};
	uint32_t val;
} CR3;
typedef struct
{
	//the 16-bit visible part, i.e..
	//the selector
	union {
		uint16_t val;
		struct
		{
			uint32_t rpl : 2;	//reg priority level
			uint32_t ti : 1;	 //0 if the GDT
			uint32_t index : 13; //table index,selector
		};
	};

	//the invisible part, i.e.. cache part
	struct
	{
		uint32_t base; //The beginning of the seg
		uint32_t limit;
		uint32_t type : 5;
		uint32_t privilege_level : 2;
		uint32_t soft_use : 1;
	};
} SegReg;
// define the structure of registers
typedef struct
{

	// general purpose registers
	union {
		union {
			union {			  //use the same storage space
				uint32_t _32; //complete 32 bit
				uint16_t _16;
				uint8_t _8[2];
			};
			uint32_t val;
		} gpr[8]; //8 registers
		struct
		{ // do not change the order of the registers
			uint32_t eax, ecx, edx, ebx, esp, ebp, esi, edi;
		};
	};

	// EIP
	uint32_t eip;

	// EFLAGS
	union {
		struct
		{
			uint32_t CF : 1;
			uint32_t dummy0 : 1;
			uint32_t PF : 1;
			uint32_t dummy1 : 1;
			uint32_t AF : 1;
			uint32_t dummy2 : 1;
			uint32_t ZF : 1;
			uint32_t SF : 1;
			uint32_t TF : 1;
			uint32_t IF : 1;
			uint32_t DF : 1;
			uint32_t OF : 1;
			uint32_t OLIP : 2;
			uint32_t NT : 1;
			uint32_t dummy3 : 1;
			uint32_t RF : 1;
			uint32_t VM : 1;
			uint32_t dummy4 : 14;
		};
		uint32_t val;
	} eflags;
#ifdef IA32_SEG
	GDTR gdtr; // GDTR, todo: define type GDTR
	// segment registers, todo: define type SegReg
	union {
		SegReg segReg[6];
		struct
		{
			SegReg es, cs, ss, ds, fs, gs;
		};
	};
	// SegDesc segDesc[6];
	// control registers
	CR0 cr0;
#endif
#ifdef IA32_PAGE
	// control registers, todo: define type CR3
	CR3 cr3;
#endif

#ifdef IA32_INTR
	// interrupt
	IDTR idtr; // IDTR, todo: define type IDTR
	uint8_t intr;
#endif
} CPU_STATE;

enum
{
	REG_AL,
	REG_CL,
	REG_DL,
	REG_BL,
	REG_AH,
	REG_CH,
	REG_DH,
	REG_BH
};
enum
{
	REG_AX,
	REG_CX,
	REG_DX,
	REG_BX,
	REG_SP,
	REG_BP,
	REG_SI,
	REG_DI
};
enum
{
	REG_EAX,
	REG_ECX,
	REG_EDX,
	REG_EBX,
	REG_ESP,
	REG_EBP,
	REG_ESI,
	REG_EDI
};

enum
{
	SREG_ES,
	SREG_CS,
	SREG_SS,
	SREG_DS,
	SREG_FS,
	SREG_GS
};

extern char *reg_names_l[];
extern char *reg_names_w[];
extern char *reg_names_b[];
extern char *sreg_names[];

// print registers
void print_reg();
// get register values
int get_reg_val(const char *reg, bool *success);

#endif
